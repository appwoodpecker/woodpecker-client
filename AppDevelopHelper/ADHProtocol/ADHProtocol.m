...
@interface ADHProtocol ()<ADHGCDAsyncSocketDelegate>

@property (nonatomic, weak) ADHGCDAsyncSocket * mSocket;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *name = [NSString stringWithFormat:@"studio.lifebetter.adhprotocol%@",(nameIndex > 0)?[NSString stringWithFormat:@"%d",nameIndex]:@""];
...
{
    ADHGCDAsyncSocket * socket = session.socket;
    ADHPRequest * request = session.request;
    ADHPackage * nextPackage = [request nextPackage];
    if(nextPackage){
        NSData * data = [nextPackage packageData];
        [socket writeData:data withTimeout:kADHProtocolPackageTimeout tag:(long)session.tag];
    }else{
...
{
    ADHGCDAsyncSocket * socket = session.socket;
    [socket readDataToData:[ADHPackage kPackageEndData] withTimeout:kADHProtocolPackageTimeout tag:(long)session.tag];
}
...
- (void)socket:(ADHGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    [self performTaskInQueue:^{
        ...
        if(package){
            ...
            if(!session){
                //接收到新的remote请求，创建新session准备，socket可能为任意app
