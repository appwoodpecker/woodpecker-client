...
#import "ADHPTProtocol.h"
#import "ADHPTUSBHub.h"
#import "ADHPTDefines.h"

@class ADHPTAddress;
@protocol ADHPTChannelDelegate;

NS_ASSUME_NONNULL_BEGIN

PT_FINAL @interface ADHPTChannel : NSObject

// Delegate
@property (strong, nullable) id<ADHPTChannelDelegate> delegate;

// Communication protocol.
@property ADHPTProtocol *protocol;
...
// queue, with *delegate*.
+ (ADHPTChannel *)channelWithDelegate:(nullable id<ADHPTChannelDelegate>)delegate NS_SWIFT_UNAVAILABLE("");
...
- (id)initWithProtocol:(ADHPTProtocol *)protocol NS_SWIFT_UNAVAILABLE("");

// Initialize a new frame channel with a specific protocol and delegate.
- (id)initWithProtocol:(nullable ADHPTProtocol *)protocol delegate:(nullable id<ADHPTChannelDelegate>)delegate NS_SWIFT_NAME(init(protocol:delegate:));

// Connect to a TCP port on a device connected over USB
- (void)connectToPort:(int)port overUSBHub:(ADHPTUSBHub *)usbHub deviceID:(NSNumber *)deviceID callback:(void(^)(NSError * _Nullable error))callback NS_SWIFT_NAME(connect(to:over:deviceID:callback:));
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError * _Nullable error, ADHPTAddress *_Nullable address))callback NS_SWIFT_NAME(connect(to:IPv4Address:callback:));
...
PT_FINAL @interface ADHPTAddress : NSObject
...
@protocol ADHPTChannelDelegate <NSObject>

@required
// Invoked when a new frame has arrived on a channel.
- (void)ioFrameChannel:(ADHPTChannel *)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(nullable NSData *)payload NS_SWIFT_NAME(channel(_:didRecieveFrame:tag:payload:));

@optional
// Invoked to accept an incoming frame on a channel. Reply NO ignore the
// incoming frame. If not implemented by the delegate, all frames are accepted.
- (BOOL)ioFrameChannel:(ADHPTChannel *)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize NS_SWIFT_NAME(channel(_:shouldAcceptFrame:tag:payloadSize:));;

// Invoked when the channel closed. If it closed because of an error, *error* is
// a non-nil NSError object.
- (void)ioFrameChannel:(ADHPTChannel *)channel didEndWithError:(nullable NSError *)error NS_SWIFT_NAME(channelDidEnd(_:error:));

// For listening channels, this method is invoked when a new connection has been
// accepted.
- (void)ioFrameChannel:(ADHPTChannel *)channel didAcceptConnection:(ADHPTChannel *)otherChannel fromAddress:(ADHPTAddress *)address NS_SWIFT_NAME(channel(_:didAcceptConnection:from:));
