...
#pragma mark -
@implementation ADHPTChannel

@synthesize protocol = protocol_;

+ (ADHPTChannel*)channelWithDelegate:(id<ADHPTChannelDelegate>)delegate {
  return [[ADHPTChannel alloc] initWithProtocol:nil delegate:delegate];
}


- (id)initWithProtocol:(ADHPTProtocol *)protocol delegate:(id<ADHPTChannelDelegate>)delegate {
  if (!(self = [super init])) return nil;
	protocol_ = protocol ? protocol : [ADHPTProtocol sharedProtocolForQueue:dispatch_get_main_queue()];
  self.delegate = delegate;
  return self;
}


- (id)initWithProtocol:(ADHPTProtocol *)protocol {
	return [self initWithProtocol:protocol delegate:nil];
}


- (id)init {
  return [self initWithProtocol:[ADHPTProtocol sharedProtocolForQueue:dispatch_get_main_queue()] delegate:nil];
}
...
- (id<ADHPTChannelDelegate>)delegate {
  return delegate_;
}


- (void)setDelegate:(id<ADHPTChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
...
- (void)connectToPort:(int)port overUSBHub:(ADHPTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^)(NSError *error))callback {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, ADHPTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
  ...
  // Success
  NSError *err = nil;
  ADHPTAddress *ptAddr = [[ADHPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  if (delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
    ADHPTChannel *peerChannel = [[ADHPTChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block ADHPTChannel *localChannelRef = self;
    ...
    ADHPTAddress *address = [[ADHPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel error:(__autoreleasing NSError**)error {
  ...
  [protocol_ readFramesOverChannel:channel onFrame:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize, dispatch_block_t resumeReadingFrames) {
    if (handleError(error, type == ADHPTFrameTypeEndOfStream)) {
      return;
    }
