...
NSString * const ADHPTProtocolErrorDomain = @"PTProtocolError";
...
static void _release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
      ADHPTProtocol *protocol = (__bridge_transfer id)objcobj;
    protocol->queue_ = NULL;
  }
}


@interface RQueueLocalIOFrameProtocol : ADHPTProtocol
@end
...
@implementation ADHPTProtocol


+ (ADHPTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
  static const char currentQueueFrameProtocolKey;
    ADHPTProtocol *currentQueueFrameProtocol = (__bridge ADHPTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey);
  if (!currentQueueFrameProtocol) {
    ...
    return (__bridge ADHPTProtocol*)dispatch_queue_get_specific(queue, &currentQueueFrameProtocolKey); // to avoid race conds
  } else {
...
- (void)readFrameOverChannel:(dispatch_io_t)channel callback:(void(^)(NSError *error, uint32_t frameType, uint32_t frameTag, uint32_t payloadSize))callback {
  ...
  dispatch_io_read(channel, 0, sizeof(PTFrame), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    if (done) {
      ...
      if (dataSize == 0) {
        callback(nil, ADHPTFrameTypeEndOfStream, 0, 0);
        return;
      }
      
      if (!allData || dispatch_data_get_size(allData) < sizeof(PTFrame)) {
        callback([[NSError alloc] initWithDomain:ADHPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
        return;
      }
...
- (void)readFramesOverChannel:(dispatch_io_t)channel onFrame:(void(^)(NSError*, uint32_t, uint32_t, uint32_t, dispatch_block_t))onFrame {
  [self readFrameOverChannel:channel callback:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize) {
    onFrame(error, type, tag, payloadSize, ^{
      if (type != ADHPTFrameTypeEndOfStream) {
...
@implementation NSData (ADHPTProtocol)
...
@implementation NSDictionary (ADHPTProtocol)
