...
NSString * const ADHPTUSBHubErrorDomain = @"PTUSBHubError";
...
NSNotificationName const ADHPTUSBDeviceDidAttachNotification = @"PTUSBDeviceDidAttachNotification";
NSNotificationName const ADHPTUSBDeviceDidDetachNotification = @"PTUSBDeviceDidDetachNotification";

ADHPTUSBHubNotificationKey const ADHPTUSBHubNotificationKeyDeviceID = @"DeviceID";
ADHPTUSBHubNotificationKey const ADHPTUSBHubNotificationKeyMessageType = @"MessageType";
ADHPTUSBHubNotificationKey const ADHPTUSBHubNotificationKeyProperties = @"Properties";
...
@implementation ADHPTUSBHub


+ (ADHPTUSBHub*)sharedHub {
  static ADHPTUSBHub *gSharedHub;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gSharedHub = [ADHPTUSBHub new];
...
- (void)connectToDevice:(NSNumber*)deviceID port:(int)port onStart:(void(^)(NSError*, dispatch_io_t))onStart onEnd:(void(^)(NSError*))onEnd {
  ...
  NSDictionary *packet = [PTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeConnect
                                                             payload:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                      deviceID, ADHPTUSBHubNotificationKeyDeviceID,
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
  NSString *messageType = [packet objectForKey:ADHPTUSBHubNotificationKeyMessageType];
  
  if ([@"Attached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ADHPTUSBDeviceDidAttachNotification object:self userInfo:packet];
  } else if ([@"Detached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ADHPTUSBDeviceDidDetachNotification object:self userInfo:packet];
  } else {
...
+ (NSDictionary*)packetDictionaryWithPacketType:(NSString*)messageType payload:(NSDictionary*)payload {
  ...
  if (bundleName) {
    packet = [NSDictionary dictionaryWithObjectsAndKeys:
              messageType, ADHPTUSBHubNotificationKeyMessageType,
              bundleName, @"ProgName",
              bundleVersion, @"ClientVersionString",
              nil];
  } else {
    packet = [NSDictionary dictionaryWithObjectsAndKeys:messageType, ADHPTUSBHubNotificationKeyMessageType, nil];
  }
...
- (BOOL)errorFromPlistResponse:(NSDictionary*)packet error:(NSError**)error {
  if (!*error) {
    ...
    if (!n) {
      *error = [NSError errorWithDomain:ADHPTUSBHubErrorDomain code:(n ? n.integerValue : 0) userInfo:nil];
      return NO;
    }
    ...
    if (replyCode != 0) {
      ...
      *error = [NSError errorWithDomain:ADHPTUSBHubErrorDomain code:replyCode userInfo:[NSDictionary dictionaryWithObject:errmessage forKey:NSLocalizedDescriptionKey]];
      return NO;
    }
  }
  return YES;
  ...
  dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    off_t offset = sizeof(ref_upacket.size);
      ...
      if (upacket_len > kUsbmuxPacketMaxPayloadSize) {
        callback(
          [[NSError alloc] initWithDomain:ADHPTUSBHubErrorDomain code:1 userInfo:@{
      ...
      if (upacket->protocol != USBMuxPacketProtocolPlist) {
        callback([[NSError alloc] initWithDomain:ADHPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package protocol" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
      ...
      if (upacket->type != USBMuxPacketTypePlistPayload) {
        callback([[NSError alloc] initWithDomain:ADHPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package type" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
