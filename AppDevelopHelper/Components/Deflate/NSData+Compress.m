...
- (NSData *)inflateWithEncodeName:(NSString *)name {
    NSData *body = self;
    if([name containsString:@"gzip"]) {
        //gzip or x-gzip
        body = [body gzipInflate];
    }else if([name containsString:@"deflate"]) {
        //zlib
        body = [body zlibInflate];
...
- (void)inflateWithEncodeName: (NSString *)name onFinish: (void(^)(NSData *))finishBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSData * body = [self inflateWithEncodeName:name];
