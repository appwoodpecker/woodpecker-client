...
NSString *const kXYStoreWillAddPaymentNotification = @"kXYStoreWillAddPaymentNotification";
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions {
    for (SKPaymentTransaction *transaction in transactions) {
        [self paymentQueue:queue updatedTransaction:transaction];
    }
}

- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransaction:(SKPaymentTransaction *)transaction {
...
- (BOOL)paymentQueue:(SKPaymentQueue *)queue shouldAddStorePayment:(SKPayment *)payment forProduct:(SKProduct *)product {
    ...
    if(product && product.productIdentifier) {
        info[@"product"] = product.productIdentifier;
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:kXYStoreWillAddPaymentNotification object:self userInfo:info];
    return NO;
}
...
- (void)finishTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    XYAddPaymentParameters *parameters = [self popAddPaymentParametersForIdentifier:productIdentifier];
    if (parameters.successBlock != nil)
    {
        parameters.successBlock(transaction);
    }
