...
NSString *const kZJReceiptVerifiedNotification = @"ZJReceiptVerifiedNotification";
...
- (BOOL)checkIsSubscribed:(XYiTunesResponse *)iTunesResponse productId:(NSString *)productId {
    ...
    for (XYInAppReceipt *appReceipt in iTunesResponse.latest_receipt_info) {
        ...
        if ([expires_date isKindOfClass:NSNull.class]) {
                  ...
                  failure:(void (^)(NSError *error))failureBlock
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //检查是否有null值，否则存储到userdefault会崩溃
            responseJSON = [self removeNullFrom:responseJSON];
            if (!responseJSON) {
                    ...
                    response:(NSDictionary *)response
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kZJReceiptVerifiedNotification object:self userInfo:info];
}
...
- (NSObject *)removeNullFrom:(NSDictionary *)object
{
